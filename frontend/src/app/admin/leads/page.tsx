  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <PageHeader
          title="All Leads"
          description="View and manage leads across your entire sales team."
        ...
        {loading ? (
          ...
          <EmptyState title="No leads found" description="Seed data or add leads to get started." />
        ) : (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50/80 border-b border-slate-100">
                  <tr>
                    {["Title", "Company", "Owner", "Value", "Status", "Actions"].map((h) => (
                ...
                <tbody className="divide-y divide-slate-50">
                  {leads.map((lead) => (
                    <tr key={lead._id} className="table-row-hover transition">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-500">{lead.owner?.name ?? "—"}</td>
