        ...
        <div className="flex justify-between items-start flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              View and manage leads across your entire sales team.
            </p>
          </div>
                ...
                {leads.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-gray-500 text-sm">
                      No leads found. Seed data or add leads to get started.
                    </td>
                  </tr>
                ) : (
                  leads.map((lead) => (
                    <tr key={lead._id} className="border-b hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{lead.owner?.name ?? "—"}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">${lead.value.toLocaleString()}</td>
                      <td className="px-6 py-4 text-sm">
                        <span className="inline-block px-2.5 py-1 bg-blue-100 text-blue-800 rounded-full text-xs capitalize">
                          {lead.status}
                        </span>
                      </td>
